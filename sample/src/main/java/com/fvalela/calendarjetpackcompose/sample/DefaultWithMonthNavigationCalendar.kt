...
class DefaultWithMonthNavigationCalendar : AppCompatActivity() {
            ...
            CalendarJetpackComposeSampleTheme(darkTheme = false) {
                Surface(
                    ...
                    Column(Modifier.fillMaxSize()) {
                        SingleMonthCalendarHelper(viewModel = viewModel)
                        DatesList(viewModel)
                    }
                }
            }
...
@Composable
private fun SingleMonthCalendarHelper(viewModel: CalendarViewModel) {
